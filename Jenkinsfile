...
def buildAndPushLibraryImage(String dockerfile, String repo, String from, List<String> tags) {
  for (String tag : tags) {
    buildAndPushImage(dockerfile, repo, from, tag, ["FROM": "${from}:${tag}"])
  }

  def latestTag=tags[tags.size() - 1]
  sh """
    docker tag "${repo}/${from}:${latestTag}" "${repo}/${from}:latest"
    if [ "\${GIT_BRANCH}" = "master" ]; then
      \${DOCKERTOOLS_PATH}/dockerw push "${repo}/${from}" "latest"
    fi
  """
}

def buildAndPushImage(String dockerfile, String repo, String from, String tag, Map<String, String> buildArgs = [:]) {
  def dockerBuildArgs = buildArgs.collect{ k, v -> "--opt \"build-arg:${k}=${v}\"" }.join(" ")
  sh """
    \${DOCKERTOOLS_PATH}/dockerw build  "${repo}/${from}" "${tag}" "${dockerfile}" "." ${dockerBuildArgs}
    if [ "\${GIT_BRANCH}" = "master" ]; then
      \${DOCKERTOOLS_PATH}/dockerw push "${repo}/${from}" "${tag}"
    fi
  """
}